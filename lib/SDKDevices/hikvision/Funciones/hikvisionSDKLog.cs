...
namespace CorsinfSDKHik.Funciones
{
    public class hikvisionSDKLog
    {
        ...
        public String buscarLogs(int m_UserID, String fechaInit,String fechaFin,String ip,string port)
        {
            ...
            struCond.struStartTime.dwYear = dateTimeStart.Year;
            struCond.struStartTime.dwMonth = dateTimeStart.Month;
            struCond.struStartTime.dwDay = dateTimeStart.Day;
            struCond.struStartTime.dwHour = 01;
            struCond.struStartTime.dwMinute = 0;
            struCond.struStartTime.dwSecond = 0;

            struCond.struEndTime.dwYear = dateTimeEnd.Year;
            struCond.struEndTime.dwMonth = dateTimeEnd.Month;
            struCond.struEndTime.dwDay = dateTimeEnd.Day;
            ...
            if (-1 == m_lGetAcsEventHandle)
            {
                ...
                msj = "NET_DVR_StartRemoteConfig FAIL, ERROR CODE" + CHCNetSDK.NET_DVR_GetLastError().ToString();
                return msj;
            }

            String _resultado = "";

            m_pDisplayListThread = new Thread(() =>
            {
                _resultado = ProcessEvent();
            });
            m_pDisplayListThread.Start();
            Marshal.FreeHGlobal(ptrCond);
            m_pDisplayListThread.Join();
        ...
        public String ProcessEvent()
        {
            ...
            String msj = "";
            Boolean Flag = true;
            CHCNetSDK.NET_DVR_ACS_EVENT_CFGG struCFG = new CHCNetSDK.NET_DVR_ACS_EVENT_CFGG();
        ...
        private String ProcessAcsEvent(ref CHCNetSDK.NET_DVR_ACS_EVENT_CFGG struCFG, ref bool flag)
        {
            try
            {
                return AddAcsEventToList(struCFG);
            }
            catch
            {
        ...
        private string  AddAcsEventToList(CHCNetSDK.NET_DVR_ACS_EVENT_CFGG struEventCfg)
        {
            String byCardNo = System.Text.Encoding.UTF8.GetString(struEventCfg.struAcsEventInfo.byCardNo).TrimEnd('\0');
            String dwCardReaderNo = struEventCfg.struAcsEventInfo.dwCardReaderNo.ToString();
            String dwDoorNo = struEventCfg.struAcsEventInfo.dwDoorNo.ToString();
            String wAccessChannel = struEventCfg.struAcsEventInfo.wAccessChannel.ToString();
            String wLocalControllerID = struEventCfg.struAcsEventInfo.wLocalControllerID.ToString();
            string byInternetAccess = struEventCfg.struAcsEventInfo.byInternetAccess.ToString();
