...
class actualizar_datos
{
	...
	function colores($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_colores SET ESTADO = 'I'";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= " UPDATE ac_colores SET ESTADO = 'A', DESCRIPCION = '" . $value['Descrip'] . "' WHERE CODIGO = '" . $value['Codigo'] . "'";
			} else {
				$campos .= " INSERT INTO ac_colores (CODIGO,DESCRIPCION)VALUES('" . $value['Codigo'] . "','" . $value['Descrip'] . "')";
			}
		}
	...
	function marcas($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_marcas SET ESTADO = 'I'";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= " UPDATE ac_marcas SET ESTADO = 'A', DESCRIPCION = '" . $value['Descrip'] . "' WHERE CODIGO = '" . $value['Codigo'] . "'";
			} else {
				$campos .= " INSERT INTO ac_marcas (CODIGO,DESCRIPCION)VALUES('" . $value['Codigo'] . "','" . $value['Descrip'] . "')";
			}
			if ($count == 300) {
				$ret = $this->db->sql_string($campos);
				$campos = '';
	...
	function emplazamiento($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_localizacion SET ESTADO = 'I';";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= "UPDATE ac_localizacion SET CENTRO='" . $value['centro'] . "',DENOMINACION='" . $value['deno'] . "',FAMILIA='" . $value['fami'] . "',SUBFAMILIA='" . $value['sub'] . "',ESTADO = 'A' WHERE EMPLAZAMIENTO = '" . $value['empla'] . "';";
			} else {
	...
	function custodio($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE th_personas SET ESTADO = 'I';";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= "UPDATE th_personas SET PERSON_NOM='" . $value['nombre'] . "',PERSON_CI='" . $value['cedula'] . "',PERSON_CORREO='" . $value['correo'] . "',PUESTO='" . $value['puesto'] . "',UNIDAD_ORG='" . $value['unidad'] . "',ESTADO='A' WHERE PERSON_NO ='" . $value['no'] . "';";
			} else {
				$campos .= "INSERT INTO th_personas (PERSON_NO,PERSON_NOM,PERSON_CI,PERSON_CORREO,PUESTO,UNIDAD_ORG) VALUES ('" . $value['no'] . "','" . $value['nombre'] . "','" . $value['cedula'] . "','" . $value['correo'] . "','" . $value['puesto'] . "','" . $value['correo'] . "');";
			}
			if ($count == 300) {
				$ret = $this->db->sql_string($campos);
				$campos = '';
	...
	function proyectos($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_proyecto SET ESTADO = 'I';";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= "UPDATE ac_proyecto SET entidad_cp='" . $value['entidad'] . "',denominacion='" . $value['deno'] . "',descripcion='" . $value['desc'] . "',validez_de=" . $value['f1'] . ",validez_a=" . $value['f2'] . ",expiracion=" . $value['f3'] . ",ESTADO='A' WHERE programa_financiacion ='" . $value['codigo'] . "';";
			} else {
				$campos .= "INSERT INTO ac_proyecto (programa_financiacion,entidad_cp,denominacion,descripcion,validez_de,validez_a,expiracion) VALUES ('" . $value['codigo'] . "','" . $value['entidad'] . "','" . $value['deno'] . "','" . $value['desc'] . "'," . $value['f1'] . "," . $value['f2'] . "," . $value['f3'] . ");";
			}
			if ($count == 300) {
				$ret = $this->db->sql_string($campos);
				$campos = '';
	...
	function genero($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_genero SET ESTADO = 'I';";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= " UPDATE ac_genero SET ESTADO = 'A', DESCRIPCION = '" . $value['Descrip'] . "' WHERE CODIGO = '" . $value['Codigo'] . "'";
			} else {
				$campos .= " INSERT INTO ac_genero (CODIGO,DESCRIPCION)VALUES('" . $value['Codigo'] . "','" . $value['Descrip'] . "')";
			}
			if ($count == 300) {
				$ret = $this->db->sql_string($campos);
				$campos = '';
	...
	function estado($part, $partes, $totalReg, $ini, $fin)
	{
		...
		$campos = " UPDATE ac_estado SET ESTADO = 'I';";
		$dato = array();
		$count = 0;
		$insert = '';
		for ($i = $ini; $i < $fin; $i++) {
		...
		foreach ($dato as $key => $value) {
			...
			if (count($existe) > 0) {
				$campos .= " UPDATE ac_estado SET ESTADO = 'A', DESCRIPCION = '" . $value['Descrip'] . "' WHERE CODIGO = '" . $value['Codigo'] . "'";
			} else {
				$campos .= " INSERT INTO ac_estado (CODIGO,DESCRIPCION)VALUES('" . $value['Codigo'] . "','" . $value['Descrip'] . "')";
			}
			if ($count == 300) {
				$ret = $this->db->sql_string($campos);
				$campos = '';
