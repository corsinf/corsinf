		...
		{		
		      if ($parametros['id']==1) {
		      	return $resp = $this->ejecutar_sp($parametros);
					...
					{
					  $sql = "DELETE FROM COLORES; DBCC CHECKIDENT (COLORES, RESEED, 0);";
				...
				if(isset($_SESSION['COLORES']))
	             ...
	             return $res;
	          		...
	          		if($parametros['primera_vez']=='true')
					{
					  $sql = "DELETE FROM PERSON_NO; DBCC CHECKIDENT (PERSON_NO, RESEED, 0);";
					  $this->db->sql_string($sql);
					}		
	             ...
	             return $res;
          		...
          		if($parametros['primera_vez']=='true')
				{
				  $sql = "DELETE FROM ESTADO; DBCC CHECKIDENT (ESTADO, RESEED, 0);";
				  $this->db->sql_string($sql);
				}
				if(isset($_SESSION['ESTADOS']))
	             ...
	             return $res;
	          		...
	          		if($parametros['primera_vez']=='true')
					{
					  $sql = "DELETE FROM GENERO; DBCC CHECKIDENT (GENERO, RESEED, 0);";
					  $this->db->sql_string($sql);
					}		
	             ...
	             return $res;
	          		...
	          		if($parametros['primera_vez']=='true')
					{
					  $sql = "DELETE FROM LOCATION; DBCC CHECKIDENT (LOCATION, RESEED, 0);";
					  $this->db->sql_string($sql);
					}		
	             ...
	             return $res;
	          	...
	          	if($parametros['primera_vez']=='true')
					{
					  $sql = "DELETE FROM MARCAS; DBCC CHECKIDENT (MARCAS, RESEED, 0);";
					  $this->db->sql_string($sql);
					}
				if(isset($_SESSION['MARCAS']))
	             ...
	             return $res;
	          		...
	          		if($parametros['primera_vez']=='true')
					{
					  $sql = "DELETE FROM PROYECTO; DBCC CHECKIDENT (PROYECTO, RESEED, 0);";
					  $this->db->sql_string($sql);
					}
   ...
   //       array(&$primera, SQLSRV_PARAM_IN),
	...
	function custodio($datos,$parametros)
	{
		...
		{ 
			$existe = $this->custodio->buscar_custodio_($_SESSION['CUSTODIO'][$i]['no']);
		   	if(count($existe)>0)
		   	{
		   	$campos.= "UPDATE PERSON_NO SET PERSON_NOM='".$_SESSION['CUSTODIO'][$i]['nombre']."',PERSON_CI='".$_SESSION['CUSTODIO'][$i]['cedula']."',PERSON_CORREO='".$_SESSION['CUSTODIO'][$i]['correo']."',PUESTO='".$_SESSION['CUSTODIO'][$i]['puesto']."',UNIDAD_ORG='".$_SESSION['CUSTODIO'][$i]['unidad']."',ESTADO='A' WHERE PERSON_NO ='".$_SESSION['CUSTODIO'][$i]['no']."';";
		   	if($j==200)
		   		{		   			
		   	...
		   	}else
		    {
		    	if($insert=='')
		    	{
		    		$insert.= "INSERT INTO PERSON_NO (PERSON_NO,PERSON_NOM,PERSON_CI,PERSON_CORREO,PUESTO,UNIDAD_ORG) VALUES ('".$_SESSION['CUSTODIO'][$i]['no']."','".$_SESSION['CUSTODIO'][$i]['nombre']."','".$_SESSION['CUSTODIO'][$i]['cedula']."','".$_SESSION['CUSTODIO'][$i]['correo']."','".$_SESSION['CUSTODIO'][$i]['puesto']."','".$_SESSION['CUSTODIO'][$i]['correo']."')";
		    	}else
		    	{
	...
	function emplazamiento($datos,$parametros)
	{
		...
		{ 
				$existe = $this->localizacion->buscar_localizacion_codigo($_SESSION['EMPLAZAMIENTO'][$i]['empla']);
			   	if(count($existe)>0)
			   	{
			   	$campos.= "UPDATE LOCATION SET CENTRO='".$_SESSION['EMPLAZAMIENTO'][$i]['centro']."',DENOMINACION='".$_SESSION['EMPLAZAMIENTO'][$i]['deno']."',FAMILIA='".$_SESSION['EMPLAZAMIENTO'][$i]['fami']."',SUBFAMILIA='".$_SESSION['EMPLAZAMIENTO'][$i]['sub']."',ESTADO = 'A' WHERE EMPLAZAMIENTO = '".$_SESSION['EMPLAZAMIENTO'][$i]['empla']."';";	
			   	if($j==200)
		   		{		   			
		   			$this->db->sql_string($campos);
		   			$campos = '';
			   	...
			   	}else
			    {
			    	if($insert=='')
			    	{
			    	$insert.= " INSERT  INTO LOCATION (CENTRO,EMPLAZAMIENTO,DENOMINACION,FAMILIA,SUBFAMILIA) VALUES ('".$_SESSION['EMPLAZAMIENTO'][$i]['centro']."','".$_SESSION['EMPLAZAMIENTO'][$i]['empla']."','".$_SESSION['EMPLAZAMIENTO'][$i]['deno']."','".$_SESSION['EMPLAZAMIENTO'][$i]['fami']."','".$_SESSION['EMPLAZAMIENTO'][$i]['sub']."')";
			    	}else
			    	{
	...
	function genero($datos,$parametros)
	{
		...
		{ 
				$existe = $this->genero->buscar_genero_CODIGO($_SESSION['GENEROS'][$i]['Codigo']);
			   	if(count($existe)>0)
			   	{
			   		$campos.= " UPDATE GENERO SET ESTADO = 'A', DESCRIPCION = '".$_SESSION['GENEROS'][$i]['Descrip']."' WHERE CODIGO = '".$_SESSION['GENEROS'][$i]['Codigo']."'";
			   		if($j==200)
		   		{		   			
		   			$this->db->sql_string($campos);
		   			$campos = '';
			   	...
			   	}else
			    {
			    	if($insert=='')
			    	{
			    		$insert.= " INSERT INTO GENERO (CODIGO,DESCRIPCION)VALUES('".$_SESSION['GENEROS'][$i]['Codigo']."','".$_SESSION['GENEROS'][$i]['Descrip']."')";
			    	}else
			    	{
	...
	function marcas($datos,$parametros)
	{
		...
		{ 
			$existe = $this->marcas->buscar_marcas_codigo($_SESSION['MARCAS'][$i]['Codigo']);
		   	if(count($existe)>0)
		   	{
		   		$campos.= " UPDATE MARCAS SET ESTADO = 'A', DESCRIPCION = '".$_SESSION['MARCAS'][$i]['Descrip']."' WHERE CODIGO = ".$_SESSION['MARCAS'][$i]['Codigo'].";";
		   		if($j==200)
		   		{		   			
		   			$this->db->sql_string($campos);
		   			$campos = '';
		   	...
		   	}else
		    {
		    	if($insert=='')
		    	{
		    		$insert.= " INSERT INTO MARCAS (CODIGO,DESCRIPCION)VALUES('".$_SESSION['MARCAS'][$i]['Codigo']."','".$_SESSION['MARCAS'][$i]['Descrip']."')";
		    	}else
		    	{
	...
	function colores($datos,$parametros)
	{
		...
		{ 
			$existe = $this->color->buscar_colores_codigo($_SESSION['COLORES'][$i]['Codigo']);
		   	if(count($existe)>0)
		   	{
		   		$campos.= " UPDATE COLORES SET ESTADO = 'A', DESCRIPCION = '".$_SESSION['COLORES'][$i]['Descrip']."' WHERE CODIGO = '".$_SESSION['COLORES'][$i]['Codigo']."';";

		   	}else
		    {
		    	if($insert=='')
		    	{
		    	$insert.= " INSERT INTO COLORES (CODIGO,DESCRIPCION)VALUES('".$_SESSION['COLORES'][$i]['Codigo']."','".$_SESSION['COLORES'][$i]['Descrip']."')";
		        }else
		        {
	...
	function estado($datos,$parametros)
	{
		...
		{ 
			$existe = $this->estado->buscar_estado_CODIGO($_SESSION['ESTADOS'][$i]['Codigo']);
		   	if(count($existe)>0)
		   	{
		   		$campos.= " UPDATE ESTADO SET ESTADO = 'A', DESCRIPCION = '".$_SESSION['ESTADOS'][$i]['Descrip']."' WHERE CODIGO = '".$_SESSION['ESTADOS'][$i]['Codigo']."';";

		   	}else
		    {
		    	if($insert=='')
		    	{
		    		$insert.= " INSERT INTO ESTADO (CODIGO,DESCRIPCION)VALUES('".$_SESSION['ESTADOS'][$i]['Codigo']."','".$_SESSION['ESTADOS'][$i]['Descrip']."')";
		    	}else
		    	{
	...
	function plantilla_masiva($datos,$parametros)
	{
		...
		if(!isset($_SESSION['ACTIVOS']))
		{
			...
			for($i = $datos['inicio']+2; $i <= $datos['TotalReg']; $i++)
			{
				...
				if($codigo!='')
				{
					...
					if($objPHPExcel->getActiveSheet()->getCell('L'.$i)->getCalculatedValue()==""){	$locat = "''"; }
					else
					{
					...
					if($locat=="''"){$respuesta.= 'Localizacion no registrada en base: '.$empla.' en el articulo con asset:'.$asset.' <br>';}
					}
					if($locat=="''"){$respuesta.= 'Localizacion no registrada en base: '.$empla.' en el articulo con asset:'.$asset.' <br>';}
					//--------------------FIN LOCALIZACION ----------------
					...
					if($objPHPExcel->getActiveSheet()->getCell('N'.$i)->getCalculatedValue()==""){$perso = "''";}
					else
					{
					 ...
					 if($perso== "''"){$respuesta.= 'Custodio no encontrado en base: '.$custo.' en el articulo:'.$asset.' <br>';}
					 }
					if($perso== "''"){$respuesta.= 'Custodio no encontrado en base: '.$custo.' en el articulo:'.$asset.' <br>';}
					 ...
					 //------------------------inicio marca---------------
					  if($objPHPExcel->getActiveSheet()->getCell('P'.$i)->getCalculatedValue()==""){$eval1 = "''";}
					  else
						{
						...
						if($eval1=="''"){$respuesta.='Marca con codigo:'.$eva1.' no registrado en base en el articulo:'.$asset.' <br>';}
						 }
						if($eval1=="''"){$respuesta.='Marca con codigo:'.$eval1.' no registrado en base en el articulo:'.$asset.' <br>';}
					 //-----------------------------------fin marca-----------------
					 //--------------------------------inicio estado-----------------
					 if($objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getCalculatedValue()==""){$eval2 = "''";}
					 else
					 {
					 ...
					 if($eval2== "''"){$respuesta.='Estado con codigo: '.$eva2.' no registrada en base en el articulo:'.$asset.' <br>';}
					 }
					 if($eval2== "''"){$respuesta.='Estado con codigo: '.$eval2.' no registrada en base en el articulo:'.$asset.' <br>';}
					 //-----------------------------fin de estado--------------------------------------

					 //-----------------------------inicio de genero-----------------------------------
					 if($objPHPExcel->getActiveSheet()->getCell('R'.$i)->getCalculatedValue()==""){$eval3 = "''"; }
					 else
					 {
					 ...
					 if($eval3=="''"){$respuesta.='Genero con codigo: '.$eva3.' no registrada en base en el articulo:'.$asset.' <br>';}
					 }
					 if($eval3=="''"){$respuesta.='Genero con codigo: '.$eval3.' no registrada en base en el articulo:'.$asset.' <br>';}
					 //------------------------------fin de genero----------------------------------------

					 //-------------------------------inicio de colores-----------------------------------
					 if($objPHPExcel->getActiveSheet()->getCell('S'.$i)->getCalculatedValue()==""){$eval4 = "''";}
					 else
					 {
					 ...
					 if($eval4== "''"){$respuesta.='Color con codigo: '.$eva4.' no registrada en base en el articulo:'.$asset.' <br>';}
					 }
					 if($eval4== "''"){$respuesta.='Color con codigo: '.$eval4.' no registrada en base en el articulo:'.$asset.' <br>';}
					 //--------------------------------fin de colores------------------------------------

					 //---------------------------------´inicio proyecto------------------------------- 
					 if($objPHPExcel->getActiveSheet()->getCell('T'.$i)->getCalculatedValue()==""){$eval5 = "''";}
					 else
					 {
					 ...
					 if($eval5== "''"){$respuesta.= 'Proyecto no registrada en base: '.$eva5.' en el articulo:'.$asset.' <br>';}
					 }
					 if($eval5== "''"){$respuesta.= 'Proyecto no registrada en base: '.$eval5.' en el articulo:'.$asset.' <br>';}
					 //--------------------------------fin proyecto -------------------------------------- 
		...
		{ 
		    ...
		    {
		    	$insert_asset= " INSERT INTO ASSET (TAG_UNIQUE,TAG_ANT,TAG_SERIE)VALUES('".$_SESSION['ACTIVOS'][$i]['RFID']."','".$_SESSION['ACTIVOS'][$i]['TAG_ANTIGUO']."','".$_SESSION['ACTIVOS'][$i]['ASSET']."');";		
		   		$this->db->sql_string($insert_asset);
		    	if($insert=='')
		    	{		    		    		
		    		$ASSET = $this->activos->asset($_SESSION['ACTIVOS'][$i]['ASSET']);

		    		$insert.="INSERT INTO PLANTILLA_MASIVA (COMPANYCODE,ID_ASSET,SUBNUMBER,DESCRIPT,DESCRIPT2,MODELO,SERIE,FECHA_INV_DATE,QUANTITY,BASE_UOM,LOCATION,PERSON_NO,EVALGROUP1,EVALGROUP2,EVALGROUP3,EVALGROUP4,EVALGROUP5,ASSETSUPNO,ORIG_ASSET,ORIG_ACQ_YR,ORIG_VALUE,OBSERVACION,BAJAS,CARACTERISTICA,IMAGEN,ACTU_POR,FECHA_BAJA)VALUES(
