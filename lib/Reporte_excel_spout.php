...
{
	$parametros = $_GET;
	$reporte->Reporte_sap_total_bajas($parametros);
	// $reporte-> ejemplo();
}
...
{
	$parametros = $_GET;
	$reporte->reporte_sap_total_terceros($parametros);
	// $reporte-> ejemplo();
}
if(isset($_GET['reporte_sap_patrimoniales']))
{	
	$parametros = $_GET;
	$reporte->reporte_sap_total_patrimoniales($parametros);
	// $reporte-> ejemplo();
}
...
function Reporte_sap_total_bajas($parametros)
{
			...
			$bloque = $this->bloq;
			$inicio = 0;
			$total = $this->articulos->total_activos(false,false,false,false,false,$desde,$hasta,1,false,false);
			$total_act = number_format(($total[0]['total']/$bloque),strlen($bloque));
			// print_r($total);die();
			
			$writer = WriterEntityFactory::createXLSXWriter();
			$filePath = 'INFORME_BAJAS.xlsx';
           ...
           ->build();
			...
			if($parametros['desde']!='' && $parametros['hasta']!='')
			{
				$rangos = array('','','','','','INFORME DESDE:'.$parametros['desde'].' HASTA:'.$parametros['hasta'],'','','','');
				$rowFromValues = WriterEntityFactory::createRowFromArray($rangos,$style);
				$writer->addRow($rowFromValues);
			}

		    $cabecera1 = array('BUKRS',
				'ANLN1',
				'ANLN2',
				'TXT50',
				'TXTA50',
		...
		{
			$limite = $inicio.'-'.$bloque;
			$datos = $this->articulos->lista_articulos_sap_codigos(false,false,false,$limite,false,false,$desde,$hasta,1,false,false);
	  	 // print_r($datos);die();
		  foreach ($datos as $key => $value) 
		  {
					// $fecha = $value['FECHA_INV_DATE']->format('Y-m-d');
				  $fecha='';
					...
					$fechaREF,
					$value['PERIODO'],
					$value['CLASE_MOVIMIENTO'],
					$value['MOVIMIENTO']);

				 	$rowFromValues = WriterEntityFactory::createRowFromArray($lista);
					$writer->addRow($rowFromValues);
			}
...
function Reporte_sap_total_terceros($parametros)
{
	...
	$cus ='';
		$desde = false;
			...
			$bloque = $this->bloq;
			$inicio = 0;
			$total = $this->articulos->total_activos(false,false,false,false,false,$desde,$hasta,false,1,false);
			$total_act = number_format(($total[0]['total']/$bloque),strlen($bloque));
			// print_r($total);die();
		...
		{
			$limite = $inicio.'-'.$bloque;
			$datos = $this->articulos->lista_articulos_sap_codigos(false,false,false,$limite,false,1,$desde,$hasta,false,1,false);
	  	 // print_r($datos);die();
		  foreach ($datos as $key => $value) 
		  {
					// $fecha = $value['FECHA_INV_DATE']->format('Y-m-d');
...
function Reporte_sap_total_patrimoniales($parametros)
{
			...
			$bloque = $this->bloq;
			$inicio = 0;
			$total = $this->articulos->total_activos(false,false,false,false,false,$desde,$hasta,false,false,1);
			$total_act = number_format(($total[0]['total']/$bloque),strlen($bloque));
			// print_r($total);die();
		...
		{
			$limite = $inicio.'-'.$bloque;
			$datos = $this->articulos->lista_articulos_sap_codigos(false,false,false,$limite,false,1,$desde,$hasta,false,false,1);
	  	 // print_r($datos);die();
		  foreach ($datos as $key => $value) 
		  {
					// $fecha = $value['FECHA_INV_DATE']->format('Y-m-d');
