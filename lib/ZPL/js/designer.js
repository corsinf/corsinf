...
com.logicpartners.labelDesigner = function(canvasid, labelWidth, labelHeight) {
	...
	this.addObject = function(object) {
		...
		this.updateCanvas();
		elem = this.getElementos();
		console.log(elem);
	}
			
	this.deleteActiveElement = function() {
		name = this.activeElement.name.replaceAll(' ','_');
		$('#tr_'+name).remove();
		console.log(name)
	...
	//Las lineas comentadas son propiedades para la impresion
	this.generateZPL = function() {
		...
		{
			data+="^RS,,,1,2\r\n"+   // Configurar el RFID
				"^RB\r\n"+ 			 // Comando RFID para preparar el tag
				"^RFW,H,\r\n"+      // Escribir datos en el tag RFID
				"^FD^FD"+$('#txt_rfid_simple').val()+"^FS\r\n" // reemplaza con el EPC real
		}
		...
		// console.log(data)
		return { "data" : bufferData, "zpl" : data ,'dataImp':tr};
	}

	//funcion creada para ver los elementos
