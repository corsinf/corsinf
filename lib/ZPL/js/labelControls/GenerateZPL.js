...
com.logicpartners.labelControl.generatezpl = function(designer) {
	...
	this.button = $("<button class='btn btn-sm btn-primary' id='btn_zpl'>Generar ZPL</button>").css({ "line-height" : "30px" }).appendTo(this.buttonContainer)
		.on("click", function() {
			...
			var dialog = $("<div></div>").prop("title", "Generar ZPL");
			
			var output = $("<textarea id='txt_zpl'></textarea>").css({ "white-space": "nowrap", resize: "none", width: "100%", height: "100%" }).val(zpl.data + zpl.zpl).appendTo(dialog);
