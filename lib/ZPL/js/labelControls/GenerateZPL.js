...
com.logicpartners.labelControl.generatezpl = function(designer) {
	...
	this.button = $("<button class='btn btn-sm btn-primary' id='btn_zpl'>Generar ZPL</button>").css({ "line-height" : "30px" }).appendTo(this.buttonContainer)
		.on("click", function() {
			var zpl = self.designer.generateZPL();
			var dialog = $("<div></div>").prop("title", "Imprimiendo Etiqueta").prop('id','id_modal');
			
			var output = $("<textarea id='txt_zpl' style='display:none'></textarea><img src='../img/de_sistema/print.gif'/>").css({ "white-space": "nowrap", resize: "none", width: "100%", height: "100%" }).val(zpl.data + zpl.zpl).appendTo(dialog);
