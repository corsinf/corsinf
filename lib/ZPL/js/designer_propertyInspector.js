			...
			for (var key in activeElement) {
				if (!activeElement.readonly || key != "readonly" && $.inArray(key, activeElement.readonly) == -1) {
					if (getType.toString.call(activeElement[key]) != '[object Function]') {
						...
						{
							activeElement['igual'] = 0;
						}
						if($('#rbl_rfid').prop('checked')){activeElement['codi'] = 1;}else{activeElement['codi'] = 0;}
						console.log(activeElement)
				...
				if (!keys[key]) {
					...
					if (key != "readonly" && getType.toString.call(activeElement[key]) != '[object Function]') {
						...
						if(key!='igual' && key!='codi')
						{
							...
							// espacio para crear checks
							opt = '';
							if(activeElement[key]==1)
							{
								opt = 'checked';
							}
							var elementKey = $('<div>texto rfid</div>')
								.css({
									"width": "65px",
									"height": "20px",
									"border": "1px solid #AAAAAA",
							...
							elementValue = $('<label class="ps-1 pe-1 mb-1"><input class="m-1"  type="checkbox" id="rbl_igual" onclick="textos_iguales()" '+opt+' />Igual que text</label>')
							.css({
								"height": "22px",
								"line-height": "20px",
							})
						...
						else if(key=='codi')
						{
							...
							var elementKey = $('<div>Codificar</div>')
								.css({
									"width": "65px",
									"height": "20px",
									"border": "1px solid #AAAAAA",
							...
							elementValue = $('<label class="ps-1 pe-1 mb-1"><input class="m-1" type="checkbox" id="rbl_rfid" '+opt+' />Codificar RFID</label>')
							.css({
								"height": "22px",
								"line-height": "20px",
							})
						...
						var elementContainer = $('<div></div>')
								...
								.append(elementKey).append(elementValue).append(elementButton);
