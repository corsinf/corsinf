	...
	function join_tabla($tbl_principal, $iden, $tbl_ligado, $iden1)
	{
		...
		switch ($join) {
			case 'PLANTILLA_MASIVA=MARCAS':
				$valor = ' LEFT JOIN ac_marcas ' . $iden1 . ' ON ' . $iden . '.EVALGROUP1 = ' . $iden1 . '.ID_MARCA';
				break;
			case 'PLANTILLA_MASIVA=GENERO':
				$valor = ' LEFT JOIN ac_genero ' . $iden1 . ' ON ' . $iden . '.EVALGROUP3 = ' . $iden1 . '.ID_GENERO';
				break;
			case 'PLANTILLA_MASIVA=ESTADO':
				$valor = ' LEFT JOIN ac_estado ' . $iden1 . ' ON ' . $iden . '.EVALGROUP2 = ' . $iden1 . '.ID_ESTADO';
				break;
			case 'PLANTILLA_MASIVA=COLORES':
				$valor = ' LEFT JOIN ac_colores ' . $iden1 . ' ON ' . $iden . '.EVALGROUP4 = ' . $iden1 . '.ID_COLORES ';
				break;
			case 'PLANTILLA_MASIVA=PROYECTO':
				$valor = ' LEFT JOIN ac_proyecto ' . $iden1 . ' ON ' . $iden . '.EVALGROUP5 = ' . $iden1 . '.ID_PROYECTO ';
				break;
			case 'PLANTILLA_MASIVA=PERSON_NO':
				$valor = ' LEFT JOIN th_personas ' . $iden1 . ' ON ' . $iden . '.PERSON_NO = ' . $iden1 . '.ID_PERSON ';
				break;
			case 'PLANTILLA_MASIVA=LOCATION':
				$valor = ' LEFT JOIN ac_localizacion ' . $iden1 . ' ON ' . $iden . '.LOCATION = ' . $iden1 . '.ID_LOCATION ';
				break;
			case 'PLANTILLA_MASIVA=ASSET':
				$valor = ' LEFT JOIN ac_asset ' . $iden1 . ' ON ' . $iden . '.ID_ASSET = ' . $iden1 . '.ID_ASSET';
				break;
			case 'PLANTILLA_MASIVA=CLASE_MOVIMIENTO':
				$valor = ' LEFT JOIN ac_clase_movimiento ' . $iden1 . ' ON ' . $iden . '.CLASE_MOVIMIENTO = ' . $iden1 . '.CODIGO';
				break;
