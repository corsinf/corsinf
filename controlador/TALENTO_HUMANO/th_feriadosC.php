...
class th_feriadosC
{
    ...
    function insertar_editar($parametros)
    {
        ...
        if ($parametros['_id'] == '') {
            if (count($this->modelo->where('th_fer_nombre', $parametros['txt_nombre'])->listar()) == 0) {
                $datos = $this->modelo->insertar($datos);
            } else {
                return -2;
            }
        } else {
            if (count($this->modelo->where('th_fer_nombre', $parametros['txt_nombre'])->where('th_fer_id !', $parametros['_id'])->listar()) == 0) {
