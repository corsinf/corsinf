...
require_once(dirname(__DIR__, 3)  . '/modelo/TALENTO_HUMANO/CARGOS/th_cargo_reqi_area_estudioM.php');

$controlador = new th_cargo_reqi_area_estudioC();

if (isset($_GET['listar'])) {
    echo json_encode($controlador->listar($_POST['id'] ?? '', $_POST['id_area_estudio'] ?? ''));
}
...
if (isset($_GET['buscar_area_estudio_car_pla'])) {
    $parametros = array(
    ...
    $datos = $controlador->buscar_area_estudio_car_pla($parametros);
...
class  th_cargo_reqi_area_estudioC
{
    private $modelo;

    function __construct()
    {
        $this->modelo = new th_cargo_reqi_area_estudioM();
    }
    ...
    function listar_modal($id = '', $button_delete = true)
    {
        ...
        $datos = [];
        if ($id !== '') {
            $datos = $this->modelo->lista_cargo_area_estudio(null, $id);
        }
        ...
        foreach ($datos as $value) {
            $id_reg = $value['_id'];
            // Cambiado de 'area_estudio_descripcion' según tu array
                ...
                $button = <<<HTML
                                <td class="text-end pe-2 py-1" style="width: 40px;">
                                    <button type="button" 
                                            ...
                                            onclick="delete_datos_area_estudios('{$id_reg}')"
    ...
    function listar($id = '', $id_area_estudio = '')
    {
        if ($id !== '') {
            $datos = $this->modelo->lista_cargo_area_estudio(null, $id);
        }
        if ($id_area_estudio !== '') {
            $datos = $this->modelo->lista_cargo_area_estudio($id_area_estudio);
        }
        return $datos;
    }
    ...
    function insertar_editar($parametros)
    {
        ...
        $datos = array(
            ...
            array('campo' => 'th_reqia_fecha_modificacion', 'dato' => date('Y-m-d H:i:s')),
        );
    ...
    function eliminar($id)
    {
        ...
        $datos = array(
            array('campo' => 'th_reqia_id_ae', 'dato' => $id),
        );
        $datos = $this->modelo->eliminar($datos);
        return $datos;
    }

    public function buscar_area_estudio_car_pla($parametros)
    {
        ...
        $datos = $this->modelo->listar_catalogo_area_estudio($car_id, $pla_id);
