...
if (isset($_GET['buscar_trabajo'])) {
    ...
    $datos = $controlador->buscar_trabajos($parametros);
...
class cn_plaza_reqct_trabajoC
{
    ...
    function listar_modal($id = '', $button_delete = true)
    {
        $datos = [];
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_trabajo(null, $id);
        }
            ...
            return
                <<<HTML
                    ...
                    <i class='bx bx-info-circle me-2 text-secondary' style='font-size: 20px;'></i>
                        <div class="lh-1">
                            <div class="text-dark fw-bold small">Sin condición de trabajo registrada</div>
                            <div class="text-muted" style="font-size: 0.75rem;">No se ha definido la condición de trabajo.</div>
                        </div>
                    </div>
                HTML;
        }
        ...
        foreach ($datos as $value) {
            $id_reg      = $value['_id'];
            $descripcion = $value['trabajo_descripcion'];
    ...
    function listar($id = '', $trabajo_id = '')
    {
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_trabajo(null, $id);
        }
        if ($trabajo_id !== '') {
            $datos = $this->modelo->listar_plaza_trabajo($trabajo_id);
        }
        return $datos;
    }
    ...
    function insertar_editar($parametros)
    {
        ...
        $datos = array(
            ...
            array('campo' => 'cn_reqct_estado',         'dato' => 1),
            array('campo' => 'cn_reqct_fecha_creacion', 'dato' => date('Y-m-d H:i:s')),
        );

        if ($id == '') {
            $datos = $this->modelo->insertar($datos);
        } else {
            $datos[] = ['campo' => 'cn_reqct_fecha_modificacion', 'dato' => date('Y-m-d H:i:s')];
            $where[0]['campo'] = 'cn_reqct_id';
            $where[0]['dato']  = $id;
    ...
    function eliminar($id)
    {
        ...
        $datos = array(
            array('campo' => 'cn_reqct_id', 'dato' => $id),
        );
        $datos = $this->modelo->eliminar($datos);
        return $datos;
    }

    public function buscar_trabajos($parametros)
    {
        $lista = [];
        ...
        // Obtiene las condiciones de trabajo NO asignadas a la plaza
        $datos = $this->modelo->listar_trabajos_no_asignados($pla_id);
