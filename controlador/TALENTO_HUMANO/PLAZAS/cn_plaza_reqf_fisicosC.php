...
class cn_plaza_reqf_fisicosC
{
    ...
    function listar_modal($id = '', $button_delete = true)
    {
        $datos = [];
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_reqf_fisicos(null, $id);
        }
            ...
            return <<<HTML
                        <div class="d-flex align-items-center bg-light border-start border-secondary border-3 p-2 shadow-sm rounded-2">
                            ...
                            <div class="lh-1">
                                ...
                                <div class="text-muted" style="font-size: 0.75rem;">No se han definido requisitos físicos para esta plaza.</div>
                            </div>
                        </div>
                    HTML;
        }
    ...
    function listar($id = '', $reqf_id = '')
    {
        ...
        if ($reqf_id !== '') {
            $datos = $this->modelo->listar_plaza_reqf_fisicos($reqf_id);
        }
        return $datos;
    }
    ...
    function insertar_editar($parametros)
    {
        ...
        $datos = array(
            ...
            array('campo' => 'cn_reqf_estado',          'dato' => 1),
            array('campo' => 'cn_reqf_fecha_creacion',  'dato' => date('Y-m-d H:i:s')),
        );
        if ($id == '') {
            $datos = $this->modelo->insertar($datos);
        } else {
            $datos[] = ['campo' => 'cn_reqf_fecha_modificacion', 'dato' => date('Y-m-d H:i:s')];
            $where[0]['campo'] = 'cn_reqf_id';
            $where[0]['dato']  = $id;
    ...
    {
        $datos = array(
            array('campo' => 'cn_reqf_id', 'dato' => $id),
        );
    ...
    public function buscar_detalles_no_asignados($parametros)
    {
        ...
        if ($pla_id <= 0 || $id_req_fisico <= 0) {
            return $lista;
        }

        $datos = $this->modelo->listar_detalles_no_asignados($pla_id, $id_req_fisico);
        foreach ($datos as $item) {
            $lista[] = [
