...
require_once(dirname(__DIR__, 3) . '/modelo/TALENTO_HUMANO/PLAZAS/cn_plaza_reqi_area_estudioM.php');

$controlador = new cn_plaza_reqi_area_estudioC();

if (isset($_GET['listar'])) {
    echo json_encode($controlador->listar($_POST['id'] ?? '', $_POST['area_estudio_id'] ?? ''));
}
...
if (isset($_GET['buscar_area_estudio'])) {
    $parametros = array(
        'query'  => isset($_GET['q'])      ? $_GET['q']      : '',
        'pla_id' => isset($_GET['pla_id']) ? $_GET['pla_id'] : 0,
    );

    $datos = $controlador->buscar_areas_estudio($parametros);
    echo json_encode($datos);
    exit;
}


class cn_plaza_reqi_area_estudioC
{
    private $modelo;

    function __construct()
    {
        $this->modelo = new cn_plaza_reqi_area_estudioM();
    }
    ...
    function listar_modal($id = '', $button_delete = true)
    {
        $datos = [];
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_area_estudio(null, $id);
        }
        ...
        if (empty($datos)) {
            return <<<HTML
                    ...
                    <i class='bx bx-info-circle me-2 text-secondary' style='font-size: 20px;'></i>
                        <div class="lh-1">
                            <div class="text-dark fw-bold small">Sin áreas de estudio registradas</div>
                            <div class="text-muted" style="font-size: 0.75rem;">No se ha definido el área de estudio.</div>
                        </div>
                    </div>
                HTML;
        }
        ...
        foreach ($datos as $value) {
            $id_reg      = $value['_id'];
            $descripcion = $value['area_estudio_descripcion'];
                ...
                $button = <<<HTML
                                <td class="text-end pe-2 py-1" style="width: 40px;">
                                    <button type="button" 
                                            ...
                                            onclick="delete_datos_area_estudio('{$id_reg}')"
    ...
    function listar($id = '', $area_estudio_id = '')
    {
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_area_estudio(null, $id);
        }
        if ($area_estudio_id !== '') {
            $datos = $this->modelo->listar_plaza_area_estudio($area_estudio_id);
        }
        return $datos;
    }
    ...
    function insertar_editar($parametros)
    {
        ...
        $datos = array(
            ...
            array('campo' => 'id_area_estudio',             'dato' => $parametros['id_area_estudio']),
            array('campo' => 'cn_reqia_estado',             'dato' => 1),
            array('campo' => 'cn_reqia_fecha_creacion',     'dato' => date('Y-m-d H:i:s')),
        );
        if ($id == '') {
            $datos = $this->modelo->insertar($datos);
        } else {
            $datos[] = ['campo' => 'cn_reqia_fecha_modificacion', 'dato' => date('Y-m-d H:i:s')];
            $where[0]['campo'] = 'cn_reqia_id_ae';
            $where[0]['dato']  = $id;
    ...
    {
        $datos = array(
            array('campo' => 'cn_reqia_id_ae', 'dato' => $id),
        );
        return $this->modelo->eliminar($datos);
    }

    public function buscar_areas_estudio($parametros)
    {
        $lista  = [];
        ...
        $datos = $this->modelo->listar_areas_no_asignadas($pla_id);
