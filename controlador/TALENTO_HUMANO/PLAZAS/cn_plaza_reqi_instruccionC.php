...
class cn_plaza_reqi_instruccionC
{
    ...
    function listar_modal($id = '', $button_delete = true)
    {
        $datos = [];
        if ($id !== '') {
            $datos = $this->modelo->listar_plaza_instruccion(null, $id);
        }
    ...
    function listar($id = '', $instruccion_id = '')
    {
        ...
        if ($instruccion_id !== '') {
            $datos = $this->modelo->listar_plaza_instruccion($instruccion_id);
        }
        return $datos;
    }
    ...
    function insertar_editar($parametros)
    {
        ...
        $datos = array(
            ...
            array('campo' => 'cn_reqi_estado',          'dato' => 1),
            array('campo' => 'cn_reqi_fecha_creacion',  'dato' => date('Y-m-d H:i:s')),
        );
        if ($id == '') {
            $datos = $this->modelo->insertar($datos);
        } else {
            $datos[] = ['campo' => 'cn_reqi_fecha_modificacion', 'dato' => date('Y-m-d H:i:s')];
            $where[0]['campo'] = 'cn_reqi_instruccion_id';
            $where[0]['dato']  = $id;
    ...
    {
        $datos = array(
            array('campo' => 'cn_reqi_instruccion_id', 'dato' => $id),
        );
    ...
    public function buscar_niveles_academicos($parametros)
    {
        ...
        $datos = $this->modelo->listar_niveles_no_asignados($pla_id);
