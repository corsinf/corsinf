...
class th_personasC
{
    ...
    function guardarImport($parametros)
    {
        ...
        foreach ($filas as $fila) {
            ...
            $loteValido[] = ['nombre' => $nombre, 'cardNo' => $cardNo];
            $nombresUnicos[$nombre] = true;
        }

        // FASE 1: Upsert en th_card_data 
        foreach ($loteValido as $r) {
            $res = $this->upsertCardPorCardNo($r['cardNo'], $r['nombre']);
            if ($res === 1)      $actualizadosCard++;
            elseif ($res === 2)  $insertadosCard++;
            else $msj .= "No se pudo procesar tarjeta para {$r['nombre']} ({$r['cardNo']}).<br>";
        }

        // FASE 2: Personas por th_per_observaciones
        // Si NO existe persona con th_per_observaciones == nombre, insertar. Si existe: NO hacer nada.
        foreach (array_keys($nombresUnicos) as $nombre) {
            $this->modelo->reset();
