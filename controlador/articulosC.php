...
class articulosC
{
	...
	function lista_articulos($parametros)
	{
		...
		$datos = $this->modelo->cantidad_registros($query,$loc,$cus);
		$total_reg = $datos[0]['numreg'];
		if($total_reg >25)
		{
		    $termino = substr($query,-1,1);
		    if($termino==','){ $query = substr($query,0,-1); }

		   $datos = $this->modelo->lista_articulos($query,$loc,$cus,$pag,false,$exacto,$asset,false,false,false,false,false,$multiple);
		}else
		{
			$termino = substr($query,-1,1);
		    if($termino==','){ $query = substr($query,0,-1); }

		   $datos = $this->modelo->lista_articulos($query,$loc,$cus,false,false,$exacto,$asset,false,false,false,false,false,$multiple);
		}
