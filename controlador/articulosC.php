...
include('../lib/pdf/cabecera_pdf.php');
include('../lib/excel_spout.php');
/**
 * 
 */
...
{
	// $parametros = $_POST['parametros'];
	echo json_encode($controlador->articulos_especiales());
}
...
class articulosC
{
  ...
  //print_r($datoss2);die();
    ...
    function articulos_especiales()
    {

    	$activos = $this->modelo->total_activo();
    	$bajas =$this->modelo->total_bajas();
    	$patrimonial = $this->modelo->total_patrimoniales();
    	$terceros = $this->modelo->total_terceros(); 
        ...
        $datos = array('activos'=>$activos[0]['cantidad'],'bajas'=>$bajas[0]['cantidad'],'patrimoniales'=>$patrimonial[0]['cantidad'],'terceros'=>$terceros[0]['cantidad'],'etiquetados'=>$eti[0]['eti'],'con_seguro'=>'','sin_seguro'=>''); 
	...
	function sql_busqueda($parametros)
	{
		...
		foreach ($tablas as $key => $value) {
			if($key!=0){
				$letra = $this->tablas_asociadas($value);
				$sql_new = str_replace(','.$value.'.',','.$letra.'.',$sql_new); 
			}else{
				$letra = $this->tablas_asociadas($tabla_pricipal);
