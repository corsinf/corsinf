...
<script>
    ...
    function cargar_ddl_plaza_estados() {
        var data_extra_aprobacion = {
            'orden': 2
        };
        var data_extra_cierre = {
            'orden': 5
        };
    ...
    function actualizar_tabs_estado(orden_actual) {
        var tabs = [{
                btn: '#btn-tab-borrador',
                orden_req: 1
            },
            {
                btn: '#btn-tab-aprobacion',
                orden_req: 2
            },
            {
                btn: '#btn-tab-publicacion',
                orden_req: 3
            },
            {
                btn: '#btn-tab-evaluacion',
                orden_req: 4
            },
            {
                btn: '#btn-tab-cierre',
    ...
    function card_colors(codigo) {
        var c = (codigo || '').trim().toUpperCase();
        if (c === 'APROBACION') return {
            bg: '#f4fdf7',
            border: '#a3cfbb',
            icon: 'bx-check-circle',
            icon_color: '#198754'
        };
        if (c === 'RECHAZADA') return {
            bg: '#fff8f8',
            border: '#f5c6cb',
            icon: 'bx-x-circle',
            icon_color: '#dc3545'
        };
        if (c === 'PROGRAMADA') return {
            bg: '#fffdf0',
            border: '#fde8a0',
            icon: 'bx-time-five',
            icon_color: '#856404'
        };
        if (c === 'PUBLICADA') return {
            bg: '#f4fdf7',
            border: '#a3cfbb',
            icon: 'bx-broadcast',
            icon_color: '#198754'
        };
        if (c === 'CONTRATADO') return {
            bg: '#f0f4ff',
            border: '#b6c8fb',
            icon: 'bx-user-check',
            icon_color: '#0d6efd'
        };
        if (c === 'DESIERTA') return {
            bg: '#fff8f8',
            border: '#f5c6cb',
            icon: 'bx-x-circle',
            icon_color: '#dc3545'
    ...
    function renderizar_historial(response) {
        ...
        if (grupos[2].length) {
            ...
            var html_ap = '';
            grupos[2].forEach(function(h) {
                html_ap += card_estado(h, !hay_evaluacion, true);
            });
        ...
        if (hay_publicacion) {
            // Ya publicada: mostrar fechas y card
            var reg_pub = grupos[3][grupos[3].length - 1];
            var fecha_pub = $('#txt_fecha_inicio').text();
            var fecha_cierre = $('#txt_fecha_cierre').text();
            ...
            $('#bloque_fechas_publicacion')
                .html('<div class="alert alert-success d-flex align-items-start gap-2 py-2 mb-3" style="border-radius:10px;">' +
                    ...
                    '&nbsp;&nbsp;' +
                    '<i class="bx bx-calendar-x me-1"></i>Cierre: <strong>' + fmt_fecha(fecha_cierre) + '</strong>' +
                    '</span></div></div>')
                .show();

            var html_pub = '';
            grupos[3].forEach(function(h) {
                html_pub += card_estado(h, !hay_evaluacion, false);
            });
            $('#hist-publicacion').html(html_pub);

        } else if (aprobada) {
            // Aprobada y no publicada: mostrar formulario
        ...
        // ── EVALUACIÓN (orden 4) ──────────────────────────────────────────
        var html_ev = '';
        grupos[4].forEach(function(h) {
            html_ev += card_estado(h, false, false);
        });
        $('#hist-evaluacion').html(html_ev);

        // ── CIERRE (orden 5) — solo disponible si hay publicación ─────────
        if (!hay_publicacion) {
        ...
        if ($cierre.val()) {
            ...
            if (fechaPub > fechaCierre) {
                ...
                Swal.fire({
                        ...
                        text: 'La fecha de publicación no puede ser mayor que la de cierre.',
    ...
    $(document).on('change', '#txt_pub_fecha_publicacion', function() {
        validarFechaPublicacion();
    });
    $(document).on('change', '#txt_pub_fecha_cierre', function() {
        validarFechaCierre_pub();
    });
    ...
    function publicar_plaza() {
        ...
        if (!fecha_pub || !fecha_cierre) {
            Swal.fire({
                ...
                text: 'Debes ingresar la fecha de publicación y la fecha de cierre.',
                confirmButtonText: 'Entendido'
            });
            return;
        }
        ...
        <div class="col-md-9 bg-white">
            <div class="tab-content p-4" id="v-pills-tabContent" style="min-height:400px;">
                ...
                <div class="tab-pane fade" id="tab-publicacion" role="tabpanel">
                    ...
                    <!-- Fechas activas cuando ya está publicada -->
                    <div id="bloque_fechas_publicacion" style="display:none;"></div>

                    <!-- Formulario para publicar (solo si está aprobada y no publicada) -->
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control form-control-sm"
                                        id="txt_pub_fecha_publicacion" name="txt_pub_fecha_publicacion" />
                                </div>
                                <div class="col-md-6">
                                    <label for="txt_pub_fecha_cierre" class="form-label fs-7 mb-1 fw-bold">Fecha de Cierre</label>
                                    <input type="date" class="form-control form-control-sm"
                                        id="txt_pub_fecha_cierre" name="txt_pub_fecha_cierre" />
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Aviso cuando está rechazada/programada -->
                ...
                <div class="tab-pane fade" id="tab-cierre" role="tabpanel">
                    ...
                    <!-- Aviso bloqueado hasta que haya publicación -->
                    <div id="bloque_aviso_cierre_bloqueado" style="display:none;"></div>

                    <!-- DDL de cierre (solo si hay publicación) -->
