...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#txt_descripcion').val(response[0].descripcion);
                 ...
                 //Selects
                 $('#ddl_tipo_reporte').append($('<option>', {
                    value: response[0].tipo_reporte,
                    text: response[0].nombre_tipo_reporte,
                    selected: true
                }));
            }
        });
    ...
    function editar_insertar() {
        ...
        var ddl_tipo_reporte = $('#ddl_tipo_reporte').val();
        var txt_descripcion = $('#txt_descripcion').val();

        var parametros = {
            '_id': '<?= $_id ?>',
            'txt_nombre': txt_nombre,
            'ddl_tipo_reporte': ddl_tipo_reporte,
            'txt_descripcion': txt_descripcion,
        };

        if ($("#form_reportes").valid()) {
    ...
    function insertar(parametros) {
        $.ajax({
            ...
            url: '../controlador/TALENTO_HUMANO/th_reportesC.php?insertar=true',
            type: 'post',
            dataType: 'json',
            ...
            success: function(response) {
                if (response == 1) {
                    Swal.fire('', 'Operacion realizada con exito.', 'success').then(function() {
                        location.href = '../vista/inicio.php?mod=<?= $modulo_sistema ?>&acc=th_reportes';
                    });
                } else if (response == -2) {
    ...
    function eliminar(id) {
        $.ajax({
            ...
            url: '../controlador/TALENTO_HUMANO/th_reportesC.php?eliminar=true',
            type: 'post',
            dataType: 'json',
            success: function(response) {
                if (response == 1) {
    ...
    function cargar_selects2() {
        url_personasC = '../controlador/TALENTO_HUMANO/th_cat_tipo_reporteC.php?buscar=true';
        cargar_select2_url('ddl_tipo_reporte', url_personasC);
    }
</script>
                    ...
                    <div class="card-body p-5">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-primary">
                                <?php
                                if ($_id == '') {
                                    echo 'Registrar Reporte';
                                } else {
                                    echo 'Modificar Reporte';
                                }
                                ?>
                            </h5>
                        ...
                        <form id="form_reportes">
                            ...
                            <div class="row pt-3 mb-col">
                                ...
                                <div class="col-sm-6">
                                    <label for="ddl_tipo_reporte" class="form-label">Tipo de Reporte </label>
                                    <select class="form-control form-control-sm select2-validation" name="ddl_tipo_reporte" id="ddl_tipo_reporte">
                                        <option value="">Seleccione</option>
                                    </select>
                                    <label class="error" style="display: none;" for="ddl_tipo_reporte"></label>
                                </div>
                            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        agregar_asterisco_campo_obligatorio('ddl_tipo_reporte');
