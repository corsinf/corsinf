...
<script type="text/javascript">
  ...
  function buscar_art() {
    $.ajax({
      // data:  {id:id},
      url: '../controlador/ACTIVOS_FIJOS/articulosC.php?tipo_view=true',
      type: 'post',
      dataType: 'json',
      success: function(response) {
        if (response == 1) {
  ...
  function ver_informe_pdf(id) {
    ...
    var url = '../controlador/ACTIVOS_FIJOS/articulosC.php?ver_pdf=true&' + filtros + '&informe=' + id + '&pag=' + $('#txt_pag').val();
    window.open(url, '_blank');
  }

  function ver_informe_excel(id) {
    var filtros = $('#pnl_filtros').serialize();
    var url = '../controlador/ACTIVOS_FIJOS/articulosC.php?ver_excel=true&' + filtros + '&informe=' + id + '&pag=' + $('#txt_pag').val();
    window.open(url, '_blank');
  }
