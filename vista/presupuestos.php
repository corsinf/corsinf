<?php 
include('./header.php');
include('../controlador/presupuestosC.php'); 
@session_start();
        ...
        // ------------------------------------------------------
         ...
         $( "#txt_nombre_cli" ).autocomplete({
            source: function( request, response ) {
                ...
                $.ajax({
                    url: "../controlador/presupuestosC.php?search_client=true",
                    type: 'post',
                    dataType: "json",
                    data: {
                        search: request.term
...
function buscar_punto_venta(id)
{
    ...
    $.ajax({
         data:  {id:id},
         url:   '../controlador/presupuestosC.php?buscar_punto=true',
         type:  'post',
         dataType: 'json',
...
function buscar_punto_bodega(id)
{
    ...
    $.ajax({
         data:  {id:id},
         url:   '../controlador/presupuestosC.php?buscar_bodegas_punto=true',
         type:  'post',
         dataType: 'json',
           success:  function (response) {  
            console.log(response);
   ...
   function datos_cliente_nuevo(ci=false,id=false)
   {
    ...
    $.ajax({
         ...
         url:   '../controlador/presupuestosC.php?datos_cliente_nuevo=true',
         type:  'post',
         dataType: 'json',
           success:  function (response) {  
            console.log(response);
   ...
   function datos_factura()
   {
    ...
    $.ajax({
         data:  {parametros:parametros},
         url:   '../controlador/presupuestosC.php?datos_cliente=true',
         type:  'post',
         dataType: 'json',
           success:  function (response) {  
            console.log(response);
  ...
  function cargar_pedido_f()
   ...
   function existen_registros (tabla) {
    ...
    $.ajax({
         data:  {datos:datos},
         url:   '../controlador/presupuestosC.php?crear_documento=true',
         type:  'post',
         dataType: 'json',
         /*beforeSend: function () {   
              var spiner = '<div class="text-center"><img src="../../img/gif/proce.gif" width="100" height="100"></div>'     
  ...
  function habilitar_cheq_comp()
  {
    ...
    if(t[1]==0)
    {
      $('#txt_pago_com').attr('readonly',true);
      $('#txt_pago_com').val('');
    }else
    {
      $('#txt_pago_com').attr('readonly',false);
    }
  }
  function habilitar_cheq_comp_modal()
  {
    var tip = $('#ddl_tipo_pago').val();
    var t = tip.split('_');
    if(t[1]==0)
  ...
  function editar_datos_cliente()
  {
    ...
    $.ajax({
         data:  {parametros:parametros},
         url:   '../controlador/presupuestosC.php?update_cliente=true',
         type:  'post',
         dataType: 'json',
           success:  function (response) { 
            if (response==1) 
   ...
   { 
    ...
    if(punto=='')
    {
      $('#modal_punto_venta').modal('show'); $.ajax({
         // data:  {num:num},
         url:   '../controlador/presupuestosC.php?punto_venta=true',
         type:  'post',
         dataType: 'json',
         /*beforeSend: function () {   
              var spiner = '<div class="text-center"><img src="../../img/gif/proce.gif" width="100" height="100"></div>'     
...
<div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
      	<div class="row">
          <div class="col-sm-6">
            <a href="presupuestos.php" class="btn btn-success btn-sm" id="btn_">Nuevo</a>
            <button class="btn btn-default btn-sm" id="btn_" onclick="factura_imprimir()">Imprimir</button>
            <a href="presupuestos.php?numfac=<?php echo $num;?>&doc=<?php echo $doc;?>&est=F" class="btn btn-warning btn-sm" id="btn_fin">Finalizar</a>
            <button class="btn btn-primary btn-sm" id="btn_abono" onclick="Agregar_Abono('<?php echo $num?>')">Añadir abono</button>
            <select class="btn btn-warning dropdown-toggle btn-sm" id="ddl_pasar" name="ddl_pasar" style="display: none;" onchange="pasar_a_factura()">
            ...
            <?php 
               ...
               {
                echo '<option value="PR">Pasar a cotizacion</option>';
               }               
            ?>
            </select>
          </div>
          <div class="col-sm-6 text-right">
            <a href="presupuestos.php?numfac=<?php echo $num;?>&doc=<?php echo $doc;?>&est=P" class="btn btn-primary btn-sm" id="btn_editar">Editar</a>
...
<div class="modal fade" id="modal_forma_pago" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Datos de pago</h5>
      </div>
      <div class="modal-body">
        <form id="form_usuario_new">
        <div class="row">
          Monto
          <input type="text" name="txt_monto" id="txt_monto" class="form-control form-control-sm">
          Numero de cheque o comprobante  
          <input type="text" name="txt_pago_com" id="txt_pago_com" class="form-control form-control-sm">  
           Fecha
          <input type="date" name="txt_pago_fecha" id="txt_pago_fecha" class="form-control form-control-sm" readonly="" value="<?php echo date('Y-m-d');?>">        
        </div>
        </form>
      </div>
      <div class="modal-footer">
          <button type="button" class="btn btn-primary" onclick="" id="btn_opcion">Guardar</button>
          <button type="button" class="btn btn-default" data-dismiss="modal" aria-label="Close" onclick=" $('#ddl_tipo_pago').val(null).trigger('change');">Cerrar</button>
    </div>
  </div>
</div>
</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="col-sm-6">
            Forma de pago
            <select class="form-control form-control-sm" id="ddl_tipo_pago" onchange="habilitar_cheq_comp_modal()">
              <option value="">Seleccione forma de pago</option>
            </select>           
          </div>          
        </div>
